...
- $\mathbf{X}$: $n \times (k + 1)$, com $n > (k+1)$ e posto $k + 1$;

- $\boldsymbol{\beta}$: $(k + 1) \times 1$;
...
No caso anterior, a primeira equação é referente ao **modelo completo**, cujos valores de $\beta_0$ e $\beta_1$ serão diferentes de $\beta^*_0$ e $\beta^*_1$, respectivos ao **modelo reduzido**, caso retirarmos $x_2$ do modelo completo.

O exemplo a seguir é baseado em Freund e Minton (1979, p. 36-39), cujo dados estão representados a seguir.
...
Assim, o valor de $\hat {\boldsymbol{\beta}} = [\hat \beta_0,\hat \beta_1,\dots,\hat \beta_k]'$ que minimiza a soma de quadrados dos desvios é:
...
A variância estimada $s^2$ também poderia ser calculada da seguinte maneira, como realizado na @sec-estvar, realizada para o caso da regressão linear simples:
...
$$
s^2 = \frac{\mathbf{y'y} - \hat {\boldsymbol{\beta_c}'}' \mathbf{X_c'} \mathbf{y}}{n-k-1} = \frac{\text{SQResíduo}}{n-k-1}
$$
...
Mais uma vez, a variância estimada $s^2$ também poderia ser calculada da seguinte maneira, como realizado na @sec-estvar, realizada para o caso da regressão linear simples:
